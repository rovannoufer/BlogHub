...
const NotificationCard = ({ data, index, notificationState }) =>{
    ...
    let { type, comment, reply, createdAt, replied_on_comment, user, 
        user: { personal_info: { fullname, username, profile_img}}, blog: { _id, blog_id, title }
        , _id: notification_id, seen} = data;

    let { userAuth : { username: author_username, profile_img: author_profile_img, access_token }} = useContext(UserContext);

    let { notifications, setNotifications, notifications : { results, totalDocs }} = notificationState;
    ...
    const handleDelete = (comment_id, type, target) =>{
         ...
         axios.post(serverUrl+ "/delete-comment" ,{
            _id: comment_id}, {
                headers:{
        ...
        <div className={"p-6 border-b border-grey border-l-black "+  (!seen ? " border-l-2" : " ")}> 
           ...
           <div className="flex gap-5 m-5">
                ...
                <div className="w-full m-5">
                     ...
                     <Link to={`/user/${username}`} className="text-black underline"> @{ username } </Link>
...

           {
            isReplying ? 
             <div className="mt-8">
                  <NotificationCommentField _id={_id} blog_author={user} index={index} replyingTo={comment._id} setReplying={setReplying} 
                  notification_id={ notification_id } notificationData={notificationState}
                  />
             </div>
            : ""
           }
